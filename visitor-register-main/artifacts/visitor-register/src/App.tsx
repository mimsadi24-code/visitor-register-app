...
const queryClient = new QueryClient({
  defaultOptions: { queries: { staleTime: 15_000, refetchOnWindowFocus: false, retry: 1, retryDelay: 500 }, mutations: { retry: 0 } },
});
    ...
    <>
      <aside className="side-rail">
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '0 8px' }}>
          <div className="brand-mark" aria-hidden="true"><ClipboardList size={19} strokeWidth={2.5} /></div>
          <div><div className="brand-name">VISITOR REGISTER</div><div className="brand-caption">Visitor management</div></div>
        </div>
  ...
  return (
    <header className="topbar">
      <div className="mobile-brand">
        ...
        <div className="brand-name">VISITOR REGISTER</div>
      </div>
...
function useModalBackHandler(open: boolean, onClose: () => void) {
  useEffect(() => {
    if (!open) return;
    const marker = `visitor-register-modal-${Date.now()}`;
    window.history.pushState({ ...(window.history.state ?? {}), visitorRegisterModal: marker }, "");
...
function DeleteVisitorDialog({
  ...
  useModalBackHandler(true, () => { if (!isDeleting) onClose(); });
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
...
function VisitorHistory() {
  ...
  return (
    <>
      ...
      <div className="surface-header">
        <div><h2 className="surface-title" id="history-heading">Visitor history</h2><p className="surface-kicker">{search ? `Results matching “${search}”` : 'Your visitor register, at a glance.'}</p></div>
